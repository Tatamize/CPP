...
Form::Form(std::string name, int tosign, int toexecute): _name(name), _signed(0), _tosign(tosign), _toexecute(toexecute)
{
	...
	colornote(MEMO, 3, " Form constructor called", _name);
}

Form::~Form()
{
	colornote(MEMO, 3, " Form destructor called", _name);
}
...
{
	*this = source;
	colornote(MEMO, 3, " Form copy constructor called", _name);
}
...
void	Form::checkExecutable(Bureaucrat const &bc) const
{
	if (!_signed)
...
void	Form::fileErrorException::tellReason(std::string reason)
{
